...
class main(QMainWindow): # Class to represent the automation code and its settings.
    def __init__(self, usuario, senha, unidade, forma_pagamento, inserir_primeiro_cte, primeiro_cte, data_vencimento, fatura_path, cte_path, cte_lista):
        ...
        if execution == "cte number is wrong": # Stop the software if number of the file is different from its xml.
            ...
            msg = QMessageBox()
            msg.setWindowTitle("Alerta")
            msg.setText("O navegador foi encerrado pelo usuário.") # It'll be informed on the screen.
            msg.exec_()
    ...
    def run(self):
        try:
            ...
            for cte in self.cte_lista:
                ...
                dados_xml = read_xml(self.cte_path, cte)
                ...
                while True:
                    try:
                        driver.find_element(by = By.CSS_SELECTOR, value = "input#tbViewDigitalizacaoNf\:itNumeroNota").click() # Verify whether the "CT-e" was imported automatically
                        digitalizacao_automatica_cte(driver, self.unidade)
                        break
                    except:
                        try:
                            driver.find_element(by = By.CSS_SELECTOR, value = "button#btnCriarPedido").click()
                            digitalizacao_manual_cte(driver, cte, self.unidade, dados_xml)
                            break
                        except:
                            pass
            ...
            return "O navegador foi encerrado pelo usuário"
#End of class "main".